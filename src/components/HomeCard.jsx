...
async function getFacilities() {
  try {
    const res = await fetch("http://localhost:5000/facilities?limit=6", {
      cache: "no-store",
    });
...
export default async function HomeCard() {
 ...
 <div className="flex justify-end">
         <Button
        radius="full"
        className="bg-[#005836] hover:bg-[#00431a] text-white font-bold px-6 py-2 shadow-lg"
      >
        
         All Facility <FaAngleDoubleRight />
      </Button> 
 </div>
    </section>
  );
